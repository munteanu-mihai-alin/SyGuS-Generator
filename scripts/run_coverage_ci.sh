...
LCOV_OPTS=(--rc branch_coverage=0 --ignore-errors mismatch,empty,negative)

lcov --capture --directory "${BUILD_DIR}" --output-file "${BUILD_DIR}/coverage.info" "${LCOV_OPTS[@]}"
lcov --remove "${BUILD_DIR}/coverage.info" \
  '/usr/*' \
  '*/tests/*' \
  '*/build-coverage/*' \
  --output-file "${BUILD_DIR}/coverage.filtered.info" "${LCOV_OPTS[@]}"
genhtml "${BUILD_DIR}/coverage.filtered.info" --output-directory "${BUILD_DIR}/coverage-html" "${LCOV_OPTS[@]}"
